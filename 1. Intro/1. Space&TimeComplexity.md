...
![alt text](../assets/TC_graph.png)

A code might give output faster when we run the code on newer computers than older computers. Here for the same code we cannot say T.C is faster in new computer than older computer. T.C is not the measure of time taken to produce output. It is the measure of increase in no. of operations as input size increases.
